...
const adminApi = {
  ...
  // ===============================================================
  // DIRECTION REQUEST MANAGEMENT
  // ===============================================================

  /**
   * Get all direction requests with filtering
   * @param {Object} params - Query parameters for filtering (propertyId, startDate, endDate)
   * @returns {Promise} Paginated list of direction requests
   */
  getDirectionRequests: async (params = {}) => {
    try {
      ...
      const response = await api.get('/api/admin/direction-requests', { params: cleanedParams });
      return adminApi.extractData(response);
    } catch (error) {
      adminApi.handleApiError(error, 'Failed to fetch direction requests');
    }
  },
  ...
  /**
   * Get all landmarks for a property
   * @param {number} propertyId - Property ID
   * @returns {Promise} List of landmarks for the property
   */
  getLandmarksByPropertyId: async (propertyId) => {
    try {
      const response = await api.get(`/api/admin/properties/${propertyId}/landmarks`);
      return adminApi.extractData(response);
    } catch (error) {
      adminApi.handleApiError(error, 'Failed to fetch landmarks');
    }
  },

  /**
   * Create a new landmark for a property
   * @param {number} propertyId - Property ID
   * @param {Object} landmarkData - Landmark creation data
   * @returns {Promise} Created landmark
   */
  createLandmark: async (propertyId, landmarkData) => {
    try {
      const response = await api.post(`/api/admin/properties/${propertyId}/landmarks`, landmarkData);
      return adminApi.extractData(response);
    } catch (error) {
      adminApi.handleApiError(error, 'Failed to create landmark');
    }
  },
   ...
   * @param {Object} landmarkData - Landmark update data
   * @returns {Promise} Updated landmark
   */
  updateLandmark: async (landmarkId, landmarkData) => {
    try {
      const response = await api.put(`/api/admin/landmarks/${landmarkId}`, landmarkData);
      return adminApi.extractData(response);
    } catch (error) {
      adminApi.handleApiError(error, 'Failed to update landmark');
    }
  },
   ...
   * @returns {Promise} Success response
   */
  deleteLandmark: async (landmarkId) => {
    try {
      const response = await api.delete(`/api/admin/landmarks/${landmarkId}`);
      return adminApi.extractData(response);
    } catch (error) {
      adminApi.handleApiError(error, 'Failed to delete landmark');
    }
  },
  ...
  getFilteredUsers: async (params = {}) => {
    try {
      // Clean up params to remove null/undefined/empty values
      const cleanedParams = Object.entries(params).reduce((acc, [key, value]) => {
        if (value !== null && value !== undefined && value !== '') {
          acc[key] = value;
        }
      ...
      console.log('Fetching filtered users with cleaned params:', cleanedParams);
      const response = await api.get('/api/admin/users/filter', { params: cleanedParams });
  ...
  /**
   * Get all direction requests made by users
   * @returns {Promise<Array>} List of all direction requests
   */
  getAllDirectionRequests: async () => {
    try {
      const response = await api.get('/api/admin/direction-requests');
      return adminApi.extractData(response);
    } catch (error) {
      console.error('Error fetching direction requests:', error);
      adminApi.handleApiError(error);
    }
  },
};
