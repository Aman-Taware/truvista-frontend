...
const propertyApi = {
  ...
  // Request directions to a property to log user interest
  requestDirections: async (propertyId) => {
    try {
      const response = await api.post(`/api/properties/${propertyId}/request-directions`);
      return propertyApi.extractData(response);
    } catch (error) {
      console.error(`Error requesting directions for property ${propertyId}:`, error);
      throw error;
    }
  },
