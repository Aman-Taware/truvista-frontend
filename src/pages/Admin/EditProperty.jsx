    ...
    const fetchPropertyData = async () => {
      ...
      try {
        ...
        const formattedProperty = {
          ...
          address: property.address || { location: '', city: '', state: '', zipcode: '' },
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        workDone: parseInt(propertyData.workDone, 10),
        assignedUserId: propertyData.assignedUserId ? parseInt(propertyData.assignedUserId, 10) : null,
        // Ensure flats have numeric values
        flats: propertyData.flats.map(f => ({ ...f, area: parseFloat(f.area), price: parseFloat(f.price) })),
        // Mark collections for replacement
        replaceAmenities: true,
        replaceCharacteristics: true,
        replaceFlats: true
      };

      const response = await adminApi.updateProperty(id, payload);
  ...
  const getStepIcon = (icon) => {
    switch(icon) {
      ...
      case 'features':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827a1.125 1.125 0 01.26-1.431l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z" />
      ...
      case 4:
        return (
          <div className="bg-white shadow sm:rounded-md p-6">
            ...
            {/* Predefined Characteristics Suggestions */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Editing property: <span className="font-medium text-gray-700">{propertyData.name || '...'}</span></p>
          </div>
          <div className="flex gap-3">
            <Link
              to={`/admin/properties/${id}/tour`}
                  ...
                  <div className="relative flex flex-col items-center group">
                    ...
                    >
                      {currentStep > step.id ? (
                        ...
                        <div className={`${currentStep === step.id ? 'text-primary-500' : 'text-gray-500'} h-5 w-5`}>
                          {getStepIcon(step.icon)}
                        </div>
                      )}
                    </div>
                    <span className="text-xs text-center font-medium text-gray-700 mt-2 max-w-[70px] whitespace-normal">
                      {step.name}
                    </span>
                  </div>
                </li>
        ...
        {error && (
          <div className="mb-6 p-4 border-l-4 border-red-400 bg-red-50 rounded-md shadow-sm">
            <div className="flex">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mt-8 space-y-4 sm:space-y-0">
          ...
          {currentStep < steps.length ? (
            <button
              ...
              className="inline-flex justify-center items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors"
            >
              Next
            ...
            <button
              ...
              className={`inline-flex justify-center items-center px-6 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors disabled:opacity-75 disabled:cursor-not-allowed`}
            >
              {isSubmitting ? (
                <>
