...
const UsersPage = () => {
  ...
  const [filters, setFilters] = useState({ search: '', role: '', startDate: '', endDate: '' });
  ...
  }, [filters.search, filters.role, filters.startDate, filters.endDate]);

  const fetchUsers = async () => {
    if (filters.search || filters.role || filters.startDate || filters.endDate) {
                ...
                <div className="flex items-center">
                  <div className="flex-shrink-0 p-2 bg-red-50 rounded-md">
                    <svg className="h-5 w-5 text-red-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M18.364 18.364A9 9.38 0 005.636 5.636m12.728 12.728A9.337 9.337 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                    </svg>
                  </div>
                  <div className="ml-3 w-0 flex-1">
                    <dl>
